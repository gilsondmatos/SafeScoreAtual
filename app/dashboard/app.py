...
def apply_secrets_to_env() -> None:
    """Carrega st.secrets para variáveis de ambiente (só se não existir)."""
    try:
...
def collect_and_score_now() -> int:
    """Coleta (ETH → fallback mock), pontua e grava CSVs. Retorna N processados."""
    ...
    try:
        ...
        st.warning(f"Coletor ETH falhou: {e}. Usando mock para demo.")
        try:
            import importlib
    ...
    for tx in txs:
        ...
        contrib_pct = (
            {k: round((v / penalty_total) * 100, 1) for k, v in hits.items()}
            if penalty_total > 0 else {}
        )
...
st.title("SafeScore — Dashboard (funcional)")

# Ações principais (um ÚNICO botão de coleta em todo o app)
...
with col2:
    if st.button("⚡ Coletar agora (ETH)", key="collect_main"):
        ...
        if n > 0:
            ...
            st.error("Não foi possível coletar dados. Verifique ETH_RPC_URL/limites do RPC.")
            # Não interrompe a página; permite tentar novamente.

files = list_csvs()
if not files:
    st.info("Nenhum CSV encontrado em app/data. Use o botão acima para coletar.")
    st.stop()
...
fname = st.sidebar.selectbox(
    "Arquivo de transações", files, index=files.index(default_file) if default_file in files else 0
)
...
chains = sorted(df_all["chain"].dropna().unique()) if "chain" in df_all.columns else ["ETH"]
chain = st.sidebar.selectbox("Fonte (chain)", chains, index=0)
...
with c1: st.metric("Transações (filtro)", len(df))
with c2: st.metric("Média de score", round(df["score"].mean(), 1) if not df.empty else 0)
with c3:
    try:
...
with c4: st.metric("Tokens distintos", df["token"].nunique() if "token" in df.columns else 0)
...
if not df.empty and "token" in df.columns:
    ...
    st.caption("Sem dados para gráfico.")
...
if show_contrib and "contrib_pct" in df.columns:
    ...
    if rows:
        ctab = pd.DataFrame(rows).sort_values(by=["tx_id", "pct"], ascending=[True, False])
        st.dataframe(ctab, use_container_width=True, height=360)
    else:
        st.caption("Sem dados de contribuição neste recorte.")

# PDF
st.subheader("Relatório (PDF)")
alert_input = st.number_input("Limiar de alerta (score < x)", min_value=0, max_value=100, value=t_env)
use_filters = st.checkbox("Usar filtros atuais para o PDF", value=True)
